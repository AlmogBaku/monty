...
impl Opcode {
    ...
    pub const fn stack_effect(self) -> Option<i16> {
        use Opcode::{
            ...
            LoadModule, LoadNone, LoadSmallInt, LoadTrue, MakeClosure, MakeFunction, Nop, Pop, Raise, RaiseImportError,
            Reraise, ReturnValue, Rot2, Rot3, SetAdd, SetExtend, StoreAttr, StoreCell, StoreGlobal, StoreLocal,
            StoreLocalW, StoreSubscr, UnaryInvert, UnaryNeg, UnaryNot, UnaryPos, UnpackEx, UnpackSequence,
        };
        Some(match self {
            // Stack operations
            Pop => -1,
